...
export default function Notification({ nftImageUri, message, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-opacity-75 flex items-center justify-center z-50 transition-all duration-300 backdrop-blur-sm font-mono">
            <div
                className={`bg-white/75 text-gray-600 rounded-xl p-6 max-w-2xl mx-4 text-center shadow-xl transition-all duration-300 ease-in-out flex flex-col gap-5 items-center 
            ...
            >
                {message === "Level" ? <>
                    ...
                    <p className="text-gray-400 font-semibold text-lg">Keep Studying To Earn Your NFT</p>
                    <button
                        onClick={handleClose}
                        className="px-6 py-2 font-semibold text-xl text-gray-200 bg-gray-600 rounded-md  hover:bg-gray-400 transition-colors duration-200 focus:outline-none cursor-pointer flex items-center gap-2"
                    >
